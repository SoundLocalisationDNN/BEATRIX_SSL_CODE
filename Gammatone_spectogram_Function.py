...
def compute_gammatone_spectrogram(audio_segment, sample_rate, fmin, fmax, 
                                  window_time=0.1, hop_time=0.05, num_filters=100, to_db=True):
    """
    ...
    """
    #audio_segment is 2D: (n_samples, n_channels)
...
if __name__ == '__main__':
    ...
    chirp_signal = chirp(t, f0=1000, f1=8000, t1=duration, method='linear')

    #Define filter bank and spectrogram parameters
    fmin = 50          #Minimum frequency (Hz)
    fmax = 10000       #Maximum frequency (Hz)
    window_time = 0.05 #Hamming window
    hop_time = 0.025   #hop time
